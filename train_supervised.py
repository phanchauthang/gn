...
if cfg.single_positive_example == 0:
    mask_all = torch.zeros(len(doc_mask), len(doc_mask)).cuda()
   ...
   # breakpoint()
    ...
    mask_train = torch.max(mask_train, torch.eye(len(mask_train)).cuda())  
...
for epoch in range(cfg.epochs):
    ...
    loss.backward()
    optimizer.step()
    val_loss, val_acc, pred, labels, duration,_ ,_= evaluate(model,criterion,t_features, t_y_test, test_mask)
